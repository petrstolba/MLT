...
<font size="24" color="orange"> **You tell me...** </font>
...
- save the parameters and SSR of first iteration
...
5. repeat 3. and 4. until convergence / other condition
...
1. calculate the gradient of the function
    ...
    \sum_i (y_{i} - (b_{0} + b_{1}x_{i}))^{2} =$  
    $\sum_i (y_{i} - (\hat y - b_{1} \hat x) - b_{1}x_{i})^{2} = \sum_i (y_{i} - \hat y - b_{1}(x_{i} - \hat x))^{2}$
    - $\nabla SSR = \frac{\partial SSR}{\partial b_{1}} = H(b_{1})$
2. generate initial random "estimate" of slope = $b$
